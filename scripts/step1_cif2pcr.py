...
def main():
    ...
    elif reference_pcr and reference_pcr.lower() == "ceo2_noise_percent_10.pcr":
        pcr_in_subfolder = os.path.join(subfolder_path, pcr_file_name)
        try:
            from fix_ceo2_noise_percent_10_pcr import fix_ceo2_pcr
            print("Applying additional fixes for CeO2_noise_percent_10 format...")
            fix_ceo2_pcr(pcr_in_subfolder)
        except ImportError:
            print("Warning: Could not import fix_ceo2_noise_percent_10_pcr, skipping additional fixes")
    elif reference_pcr and reference_pcr.lower() == "tbbaco_noise_percent_10.pcr":
        pcr_in_subfolder = os.path.join(subfolder_path, pcr_file_name)
        try:
            from fix_tbbaco_noise_percent_10_pcr import fix_tbbaco_pcr_with_root
            print("Applying additional fixes for tbbaco_noise_percent_10 format...")
            fix_tbbaco_pcr_with_root(pcr_in_subfolder, root_dir)
        except ImportError:
            print("Warning: Could not import fix_tbbaco_noise_percent_10_pcr, skipping additional fixes")
    elif reference_pcr and reference_pcr.lower() == "ba.pcr":
        pcr_in_subfolder = os.path.join(subfolder_path, pcr_file_name)
        try:
            from reference_pcr_Ba import apply_reference_params
            print("Applying reference parameters for Ba (monoclinic) from CIF...")
            apply_reference_params(pcr_in_subfolder, root_dir, cif_name='Ba_vesta.cif')
        except ImportError:
            print("Warning: Could not import reference_pcr_Ba, skipping reference parameter application")
        try:
            from fix_pcr_Ba import fix_ba_pcr
            print("Applying additional fixes for Ba (monoclinic) format...")
            fix_ba_pcr(pcr_in_subfolder)
        except ImportError:
            print("Warning: Could not import fix_pcr_Ba, skipping additional fixes")
