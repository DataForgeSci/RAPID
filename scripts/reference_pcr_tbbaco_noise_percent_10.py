...
def extract_reference_parameters(ref_pcr_path):
    ...
    try:
        ...
        print("\nParameters extracted from reference PCR (tbbaco_noise_percent_10):")
...
def apply_parameters_to_pcr(pcr_file_path, params):
    ...
    try:
        ...
        print("Successfully applied parameters to PCR file from tbbaco_noise_percent_10.pcr")
        return True
...
def apply_reference_params_extended(pcr_file_path, root_dir, copy_asymmetry=False, copy_scan_range=False):
    ...
    try:
        ...
        ref_pcr_path = find_reference_pcr_file('tbbaco_noise_percent_10.pcr', root_dir)
        if not ref_pcr_path:
            print("Warning: Could not find reference PCR file tbbaco_noise_percent_10.pcr for additional parameters")
            return False
...
def apply_reference_params(pcr_file_path, root_dir):
    ...
    try:
        ref_pcr_path = find_reference_pcr_file('tbbaco_noise_percent_10.pcr', root_dir)
        if not ref_pcr_path:
            print("Warning: Could not find reference PCR file tbbaco_noise_percent_10.pcr")
            return False
        ...
        if result:
            print("\nReference PCR parameters applied from tbbaco_noise_percent_10.pcr:")
