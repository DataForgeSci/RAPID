...
def align_zero_parameter(lines):
    # Find the index of the zero parameter line
    zero_line_index = -1
    ...
    # Get the zero parameter value from the line
    ...
    # Calculate the shift based on the length of the zero parameter
    shift = max(0, zero_length - 1)

    # Adjust the line by removing the calculated number of spaces
    adjusted_line = lines[zero_line_index][shift:] if shift > 0 else lines[zero_line_index]

    # Additional adjustment for negative values
...
#     # Find the index of the background parameters line
...
#     # Get the background parameters line
#     background_line = lines[background_line_index]

#     # Calculate the shift based on the presence of 10th or 100th digit
...
#         # Determine the shift needed based on the length of the integer part
...
def align_background_parameter(lines):
    ...
    # Simply preserve values and ensure consistent leading spaces
    background_parts = lines[background_line_index].split()
    if background_parts:
        # Join with consistent spacing, preserving original values
        lines[background_line_index] = "   " + "  ".join(background_parts) + "\n"

    return lines

def align_lattice_parameters(lines):
    # Find the index of the lattice parameters line
    ...
    # Extract the lattice parameters and the line below it
    lattice_params_line = lines[lattice_line_index].strip()
    reference_line = lines[lattice_line_index + 1].strip()

    # Split the lines into individual components while preserving spaces
    ...
    # Create a DataFrame to align the parameters
    df = pd.DataFrame([reference_params, lattice_params]).T

    # Format the lattice parameters to match the reference line
    for i in range(len(lattice_params)):
        df.iloc[i, 1] = "{:10.6f}".format(float(lattice_params[i]))

    # Shift the entire lattice parameters line to the right by one space
...
def append_spaces_to_lattice_line(pcr_file):
    ...
    if target_line_index != -1 and target_line_index < len(lines):
        # Append three spaces to the end of the line immediately below the target line
...
def create_folders_and_move_pcr_files(subfolder_path, pcr_filenames, dataset_status):
    for pcr_filename in pcr_filenames:
        # Create folder name by removing the .pcr extension
        ...
        # Move the .pcr file into the created folder
        ...
        try:
            if os.path.exists(original_pcr_path):
                ...
                # Debugging: print which folder PCR folder has been moved to
...
def copy_paste_dat_file(subfolder_path, pcr_filename, dat_file_name, dataset_status):
    ...
    # Now we rely directly on the known dat_file_name (from parameters), 
    # not from pcr_filename or underscores.
...
def read_original_values(pcr_file_path, atom_format='4-line'):
    ...
    # Store atom data in original_values
    ...
    # For a representative Biso (for backward compatibility)
...
def modify_combined_parameters(pcr_file_path, zero_value, background_value, a_value, b_value,
                             ...
                             perturb_bov=False, bov_value=None):
    with open(pcr_file_path, 'r') as file:
        lines = file.readlines()
    ...
    # Process all atoms in the structure (only if NOT perturbing Bov)
    if atom_section_start > 0 and atom_section_end > 0 and not perturb_bov:
        # Process each atom based on lines_per_atom (2 or 4)
        current_line = atom_section_start
        atom_index = 0
        ...
        while current_line < atom_section_end:
            ...
            if re.match(r'^[A-Za-z0-9]+\s+[A-Za-z0-9]+\s+[-+]?\d*\.\d+', lines[current_line]):
                ...
                if len(parts) >= 6:
                    try:
                        # Get current Biso value and apply the atom-specific percentage shift
                        current_biso = float(parts[5])
                        
                        # Apply the specific shift for this atom
                        if isinstance(biso_shifts, list) and atom_index < len(biso_shifts):
                            # Use individual shift for this atom
                            atom_shift = biso_shifts[atom_index]
                            new_biso = current_biso * (1 + atom_shift / 100.0)
                        else:
                            # Fallback to using a single shift for all atoms
                            new_biso = current_biso * (1 + biso_shifts / 100.0)
                            
                        parts[5] = "{:.5f}".format(new_biso)
                        
                        # Adapt formatting based on the specific atom format observed
                        # Use consistent formatting for all atoms to match reference Ba.pcr format
                        if lines_per_atom == 2:
                            # For 2-line format (simplified PCR) - consistent formatting for all atoms
                            new_line = "{:<6s} {:<6s}  {:>8s}  {:>8s}  {:>8s}  {:>7s}  {:>7s}  {:>1s}   {:>1s}   {:>1s}   {:>1s}  \n".format(
                                parts[0], parts[1], parts[2], parts[3], parts[4],
                                parts[5], parts[6], parts[7], parts[8], parts[9], parts[10] if len(parts) > 10 else "0"
                            )
                        else:
                ...
                # Move to the next atom based on format (2 or 4 lines per atom)
                current_line += lines_per_atom
            else:
                current_line += 1
    ...
    for i, line in enumerate(lines):
        ...
        # Background parameter
            ...
            if bg_range == 0:
                # If bg_range is 0, preserve the exact original background value without reformatting
        ...
        # Scale and Bov parameters
        elif '!  Scale        Shape1      Bov      Str1      Str2      Str3   Strain-Model' in line:
            parts = lines[i + 1].split()

            # Check if the original value uses scientific notation (contains 'E' or 'e')
            original_value = parts[0]
            uses_scientific = 'E' in original_value or 'e' in original_value

            # Use the same notation format as the original value
            if uses_scientific:
                # Use scientific notation if original did
                parts[0] = "{:.7E}".format(float(scale_value))
            else:
                # Use regular decimal notation if original did
                parts[0] = "{:.7f}".format(float(scale_value))

            # If perturbing Bov, update Bov value (index 2)
            if perturb_bov and bov_value is not None:
                parts[2] = "{:.5f}".format(float(bov_value))
    ...
    # Generate a shorter filename to avoid path length limits
    # Use just the base PCR name and a hash of the parameter values
    import hashlib
    ...
    # Write the new PCR file
    with open(new_pcr_file_path, 'w') as file:
        file.writelines(lines)
    
    # Return the new filename for reference
...
def generate_parameter_sets_from_input(zero_range, bg_range, lattice_range, biso_range, 
    ...
    # Identify which parameters are being tweaked
    tweaked_indices = []
    ranges = []
    ...
    # Base index for additional parameters after all possible lattice parameters
    base_idx = 8
    
    # Add individual Biso parameters if their range is non-zero
    if biso_range != 0:
    ...
    # Total parameter size - allocate enough space for all parameters
    total_params = next_idx
    
    # If no parameters are being tweaked, return single point with zeros
    if not tweaked_indices:
        point = [0] * total_params  # Make room for all parameters
        return [point], [point]

    # Special handling for single parameter case
    ...
    # Handle multiple parameters case
        ...
        # Generate points using Sobol sequence or grid points
        if is_perfect_cube(uniform_count):
            ...
            # Scale points to parameter ranges
            uniform_combinations = []
...
####
## Functions to generate uniform sampling using Sobol sequence points
####
...
def generate_grid_points(N, zero_range, bg_range, lattice_a_range, *other_lattice_ranges):
    ...
    #Generalized grid point generator for any number of dimensions, ensuring uniqueness based on zero, background, and lattice parameter A.
    #:param N: The total number of points to generate.
    #:param zero_range: Range for the zero parameter.
    #:param bg_range: Range for the background parameter.
    #:param lattice_a_range: Range for lattice parameter A (common for uniqueness).
    #:param other_lattice_ranges: Ranges for other lattice parameters (e.g., b, c, alpha, beta, gamma).
    #:return: Array of points with uniqueness ensured for zero, background, and lattice A.
    
    # Calculate points per axis based on the number of dimensions
    num_points_per_axis = int(round(N ** (1 / 3)))  # We ensure the uniqueness for the first 3 dimensions (zero, bg, a)
    
    # Create evenly spaced points for zero, background, and lattice A
    ...
    # Create evenly spaced points for other lattice parameters
    other_points = [np.linspace(-r, r, num_points_per_axis) for r in other_lattice_ranges]
    
    # Generate the grid of points using Cartesian product, ensuring uniqueness for zero, background, and lattice A
    unique_points = list(itertools.product(zero_points, bg_points, lattice_a_points))
    
    # Add other lattice parameters to the grid
    other_combinations = list(itertools.product(*other_points))
    
    # Concatenate unique points with other lattice parameters, but ensure uniqueness for zero, bg, and a
    full_combinations = [point + other for point, other in zip(unique_points, itertools.cycle(other_combinations))]
    
    # Ensure exactly N points are selected
...
def generate_sobol_points_generalized(N, zero_range, bg_range, lattice_a_range, *other_lattice_ranges):
    ...
    #Generalized Sobol point generator ensuring uniqueness for zero, background, and lattice parameter A.
    #:param N: The total number of points to generate.
    #:param zero_range: Range for the zero parameter.
    #:param bg_range: Range for the background parameter.
    #:param lattice_a_range: Range for lattice parameter A (common for uniqueness).
    ...
    # Generate Sobol sequence points for the first 3 dimensions (zero, bg, lattice A)
    points = sobol_seq.i4_sobol_generate(3 + len(other_lattice_ranges), N)  # Total dimensions = 3 + other lattice params
    
    # Scale Sobol points for zero, background, and lattice A
    ...
    # Scale Sobol points for other lattice parameters
...
def generate_sobol_points(N, zero_range, bg_range, lattice_a_range):
    ...
    # Scale points to the specified dimensions
    points[:, 0] = points[:, 0] * (2 * zero_range) - zero_range
    points[:, 1] = points[:, 1] * (2 * bg_range) - bg_range
    points[:, 2] = points[:, 2] * (2 * lattice_a_range) - lattice_a_range
...
def main():
    ...
    # Get subfolder name from inputs.txt instead of hardcoding "Si_"
    ...
    # Read the Bov perturbation flag early (needed for print statements below)
    perturb_bov_flag = parameters.get('47>Give perturbations to Bov instead of Biso (Y/N)?:', 'N').strip().upper()
    perturb_bov = (perturb_bov_flag == 'Y')

    # Read original values from the .pcr file with correct atom format
    ...
    # For each atom, print its name and original Biso value (only if NOT perturbing Bov)
    if 'atoms' in original_values and not perturb_bov:
        print "Original Biso values for each atom:"
        for atom in original_values['atoms']:
            print "  {} ({}): Biso = {}".format(atom['name'], atom['type'], atom['biso'])

    # If perturbing Bov, print the original Bov value
    if perturb_bov and 'bov' in original_values:
        print "Original Bov value: {}".format(original_values['bov'])
    ...
    # Read the number of datasets to be generated
    ...
    # Print message about which parameter will be perturbed (perturb_bov was already read earlier)
    if perturb_bov:
        print "\n" + "=" * 80
        print "PERTURBATION MODE: Bov (thermal parameter)"
        print "Biso values will NOT be modified. Bov will be perturbed using biso_range."
        print "=" * 80 + "\n"
    else:
        print "\n" + "=" * 80
        print "PERTURBATION MODE: Biso (individual atom thermal parameters)"
        print "Bov will NOT be modified. Individual Biso values will be perturbed."
        print "=" * 80 + "\n"
    ...
    # Generate parameter sets with atom-specific Biso variations
    ...
    # Concatenate uniform and random sets
    parameter_sets = np.vstack((uniform_combinations, random_combinations))

    # Debugging: Log parameter sets being processed with atom-specific Biso shifts or Bov
    ...
    for idx, param_set in enumerate(parameter_sets):
        ...
        # Extract atom-specific Biso shifts
        atom_biso_shifts = param_set[8:8+num_atoms]

        # Extract other parameters
        other_params = param_set[8+num_atoms:]

        # Format basic parameters
        basic_str = "  ".join(["{:14.10f}".format(val) for val in basic_params])

        # Format thermal parameter (Bov or Biso depending on mode)
        if perturb_bov:
            # In Bov mode, only the first shift is used for Bov
            bov_shift = atom_biso_shifts[0] if len(atom_biso_shifts) > 0 else 0.0
            thermal_str = "Bov:{:8.5f}%".format(bov_shift)
        else:
            # In Biso mode, show all individual atom shifts
            thermal_str = ""
            for a_idx, atom_shift in enumerate(atom_biso_shifts):
                ...
                thermal_str += "{}:{:8.5f}%  ".format(atom_name, atom_shift)
            thermal_str = "Biso: " + thermal_str

        # Format other parameters
        other_str = "  ".join(["{:14.10f}".format(val) for val in other_params])

        print "Set {:2d}: [ {} | {} | {} ]".format(idx, basic_str, thermal_str, other_str)
    print "=" * 150
    ...
    # Debugging purpose: show number of random sampling
    ...
    # Convert random set to a NumPy array for better readability
    random_combinations_array = np.array(random_combinations)

    # Check if figure display should be disabled
    generate_figures = parameters.get('16>Generate all output figures (Y/N)?', 'Y').strip().upper()
    display_figures = parameters.get('17>Display all output figures (Y/N)?', 'Y').strip().upper()

    # Determine which parameters have non-zero ranges
    active_params = []
    if zero_range != 0:
    ...
    # Fill remaining parameters to always have 3
    ...
    # Take first three parameters
    active_params = active_params[:3]
    param_names, param_indices = zip(*active_params)

    # Plot the uniform set if figure generation is enabled
    if generate_figures == 'Y' and parameters.get('41>Generate parameter shift distribution plots (Y/N)?:', 'Y').strip().upper() == 'Y':
        ...
        # We don't need to recreate the folders here since they were created in the uniform plot section
        plt.legend()
    ...
    for idx, param_set in enumerate(parameter_sets):
        # Extract parameters from the set - use indexing instead of unpacking
        # This avoids the "too many values to unpack" error
        # Original parameters
        zero_shift = param_set[0]
        bg_shift = param_set[1]
        
        # Handle lattice parameters based on lattice type
        if lattice_type == 'cubic':
        ...
        # Calculate base parameter modifications
        ...
        # Extract other parameter shifts
        ...
        # Calculate Bov value if perturbing Bov instead of Biso
        if perturb_bov and 'bov' in original_values:
            # Use the first atom's Biso shift to perturb Bov (or average of all atom shifts)
            if isinstance(atom_biso_shifts, list) and len(atom_biso_shifts) > 0:
                # Use the first atom's shift (or you could average all shifts)
                bov_shift = atom_biso_shifts[0]
            else:
                bov_shift = 0.0
            bov_value = float(original_values['bov']) * (1 + bov_shift / 100.0)
        else:
            bov_value = None

        # Create shift strings for file naming
        ...
        # Add atom-specific Biso shift strings
        biso_shift_strs = []
        ...
        # Modify all parameters in the PCR file - now using hashed filenames
        modified_pcr_path = modify_combined_parameters(
            ...
            perturb_bov,  # Pass the perturb_bov flag
            bov_value  # Pass the calculated bov_value
        )

        # Define a simpler PCR filename based on the loop index
        sample_pcr_filename = "sample{}.pcr".format(idx + 1)

        # Rename the file on disk to the simpler name
        ...
        # Append spaces to lattice line after modifying combined parameters using the simpler name
        append_spaces_to_lattice_line(new_pcr_file_path)

        # Move and copy the final PCR file using the simpler 'sample#.pcr' name
    ...
    # Handle retry logic for missing datasets
    ...
    # Rename PCR files in each folder to sample#.pcr
    pcr_folders = []
    # Skip special folders and only collect folders with PCR files
    ...
    # Sort folders naturally to ensure correct ordering
    def natural_sort_key(s):
        return [int(t) if t.isdigit() else t.lower() for t in re.split(r'(\d+)', s)]
    pcr_folders.sort(key=natural_sort_key)
    
    # Rename both folders and PCR files
    ...
    for folder in pcr_folders:
        ...
        # Then rename the PCR file inside the renamed folder
        ...
        sample_number += 1  # Increment counter after processing each folder
