...
def parse_fullprof_out_file(out_path):
    ...
    result = {
        ...
        "biso": None,
        "bov": None,                # Overall thermal parameter (alternative to individual Biso)
        "status": "OK",
    ...
    for ln in lines:
        ...
        # Extract Bov (overall thermal parameter) if present
        if "Bov =" in ln or "Overall Biso" in ln:
            bov_match = re.findall(r"(?:Bov|Overall Biso)\s*=\s*([-\+0-9\.]+)", ln, re.IGNORECASE)
            if bov_match:
                try:
                    result["bov"] = float(bov_match[0])
                except:
                    pass
...
def create_analysis_report(refine_root, output_file, cnn_results, soln_results, omit_background=False, use_bov=False):
    ...
    with open(output_file, "w") as f:
        ...
        if use_bov:
            f.write("#   Bov           : overall thermal parameter\n")
        else:
            for atom in all_atoms:
        ...
        def format_line(label, res, biso_dict, cell_params):
            ...
            if use_bov:
                bov_val = res.get("bov")
                if bov_val is not None:
                    biso_cols = f"{bov_val:8.5f}   "
                else:
                    biso_cols = "   NA      "
            else:
                biso_cols = ""
...
def parse_pcr_parameters(pcr_path):
    params = {
        "zero": None, "biso": None, "bov": None, "scale": None,
                ...
                if data:
                    try:
                        ...
                        if 'E' in scale_str or 'e' in scale_str:
                            ...
                            params["scale"] = float(scale_str)
                        # Extract Bov (index 2: Scale Shape1 Bov ...)
                        if len(data) > 2:
                            params["bov"] = float(data[2])
                    except:
                        pass
...
def read_ml_inputs(ml_file):
    ...
    # Line 13 (index 12): Gave perturbations to Bov instead of Biso?
    use_bov = False
    if len(lines) >= 13:
        use_bov = lines[12].lower().strip() == 'y'

    return model_name, dataset_folders, experiment_base, omit_background, material_name, peak_number, zoom_width, use_bov
...
def main():
    ...
    model_name, dataset_folders, experiment_base, omit_background, material_name, peak_number, zoom_width, use_bov = read_ml_inputs(ml_inputs)
    ...
    for dataset_name in dataset_folders:
        ...
        if cnn_out_files:
            ...
            if cnn_results and refined_params:
                ...
                cnn_results["bov"] = refined_params.get("Bov")
        ...
        if soln_out_files:
            ...
            if soln_pcr_files:
                ...
                if soln_results:
                    ...
                    soln_results["bov"] = pcr_params["bov"]
        ...
        create_analysis_report(refine_root, report_path, cnn_results, soln_results, omit_background, use_bov)
