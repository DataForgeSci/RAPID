    ...
    try:
        ...
        for param in param_list:
            if param == 'zero':
                ...
                param_counts['bov'] += 1
                param_types.append('bov')  # Overall thermal parameter
...
class CNN(nn.Module):
    def __init__(self, act='relu', output_dim=8, param_info=None):
        ...
        self.bov_count = 0      # Overall thermal parameter (alternative to Biso)

        if param_info:
            self.lattice_count = param_info['param_counts'][PARAM_TYPE_LATTICE]
            self.biso_count = param_info['param_counts'][PARAM_TYPE_BISO]
            self.bov_count = param_info['param_counts'].get(PARAM_TYPE_BOV, 0)
            has_bg_in_info = param_info['has_background']
            # Thermal parameter count: use Bov if present, otherwise use Biso
            self.thermal_count = self.bov_count if self.bov_count > 0 else self.biso_count
            ...
            self.thermal_count = self.biso_count  # Use biso_count as thermal_count when no param_info
        ...
        self.biso_out = nn.Linear(32, self.thermal_count)  # Dynamic size (handles both Biso and Bov)
    ...
    def forward(self, x):
        ...
        # Thermal parameters (Biso or Bov)
        if self.thermal_count == 1:
            outputs.append(biso)
        else:
            for i in range(self.thermal_count):
    ...
    for line_num, line in enumerate(data_lines):
        ...
        if current_prefix == "10":
            for i in range(1, len(parts)):
                ...
                is_last_field = (i == len(parts) - 1)

                if value_str.endswith("10") and len(value_str) > 2 and not is_last_field:
