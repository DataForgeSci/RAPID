...
def read_ml_inputs(ml_file):
    ...
    use_bov = False
    if len(lines) >= 13:
        use_bov = lines[12].lower().strip() == 'y'
    ...
    return model_name, dataset_folders, experimental_line, refine_line, omit_background, use_bov
...
def read_refined_parameters(refined_param_file, omit_background=False):
    ...
    with open(refined_param_file, 'r') as f:
        for line in f:
            ...
            elif key == 'lattice alpha' and float(val_part.strip()) != 90.0:
                crystal_structure = "triclinic"  # Non-90 alpha
            elif key == 'lattice beta' and float(val_part.strip()) != 90.0:
                crystal_structure = "monoclinic"  # Non-90 beta
            elif key == 'lattice gamma' and float(val_part.strip()) != 90.0:
    ...
    # Check if we have Bov (overall thermal parameter) instead of individual Biso
    has_bov = False
    if biso_atoms:
    ...
    with open(refined_param_file, 'r') as f:
        for line in f:
            ...
            if key_lower.startswith("biso"):
                ...
                has_bov = True
                params["Bov"] = val
                print(f"Found Bov (overall thermal parameter) with value: {val}")
            else:
...
def create_pcr_with_cnn_params(original_pcr, new_pcr, param_dict, omit_background=False):
    ...
    for i, line in enumerate(lines):
        if ", VARY xyz" in line:
            line = line.replace(", VARY xyz", "")
            print(f"Removed 'VARY xyz' from line: {line.strip()}")
        ...
        if "!  Zero    Code    SyCos" in line:
            ...
            if i + 1 < len(lines):
                if crystal_structure == "cubic":
                    ...
                    c_val = param_dict.get('Lattice c', 5.430)
                    alpha_val = 90.0
                    beta_val = param_dict.get('Lattice beta', 90.0)
                    gamma_val = 90.0
                elif crystal_structure == "triclinic":
                    a_val = param_dict.get('Lattice a', 5.430)
                    b_val = param_dict.get('Lattice b', 5.430)
                    c_val = param_dict.get('Lattice c', 5.430)
                    alpha_val = param_dict.get('Lattice alpha', 90.0)
                    beta_val = param_dict.get('Lattice beta', 90.0)
                    gamma_val = param_dict.get('Lattice gamma', 90.0)
            ...
            if i + 1 < len(lines):
                ...
                # Check if we have Bov prediction to apply
                has_bov = param_dict.get('_has_bov', False)
                if has_bov and 'Bov' in param_dict:
                    bov_val = param_dict.get('Bov', float(existing_parts[2]))
                    new_line = f"  {scale_mantissa:7.7f}E-03   {existing_parts[1]:>7}   {bov_val:7.5f}   {existing_parts[3]:>7}   {existing_parts[4]:>7}   {existing_parts[5]:>7}       {existing_parts[6]}\n"
                    print(f"Applied Bov value: {bov_val}")
                else:
...
def main():
    ...
    model_name, dataset_folders, experimental_dat, refine_line, omit_background, use_bov = read_ml_inputs(inputs_file)
    ...
    for dataset_name in dataset_folders:
        ...
        print(f"Thermal parameter mode: {'Bov (overall)' if use_bov else 'Biso (individual atoms)'}")
